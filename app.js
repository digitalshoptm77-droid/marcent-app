...
window.onload = function() {
    ...
    window.addEventListener('fb-sync-done', function() {
        try {
            ...
            // ✅ Firebase sync শেষে শিরোনাম কার্ড refresh — পণ্য যোগ হলে lock খুলে যাবে
...
function renderProductGrid(productList, isLoadMore = false) {
    ...
    try {
        ...
        // 🔀 এলোমেলো অর্ডার — শুধুমাত্র প্রথমবার লোড হলে (isLoadMore = false)
        if (!isLoadMore) {
            for (let i = finalDisplayList.length - 1; i > 0; i--) {
                const j = Math.floor(Math.random() * (i + 1));
                [finalDisplayList[i], finalDisplayList[j]] = [finalDisplayList[j], finalDisplayList[i]];
            }
        }
...
<script>
(function(){
  var realW = screen.width;
  var renderedW = window.innerWidth || 800;
  var ratio = realW / renderedW;
  if(ratio > 1.1){
    document.documentElement.style.fontSize = Math.round(ratio * 16) + 'px';
  }
})();
</script>
<style>
...
<div style="margin-bottom:20px;">
    ...
    <small style="display:block; margin-top:6px; color:#475569; font-size:9px; font-style:italic;">* ছবির ওপর ক্লিক করলে বড় করে দেখতে পাবেন</small>
                ...
                <div id="userChatBox" style="height:320px; overflow-y:auto; background:rgba(0,0,0,0.3); border-radius:20px; padding:15px; border:1px solid #1e293b; display:flex; flex-direction:column; gap:12px; margin-bottom:10px;">
                    ${(ret.messages || []).map(m => `
                        <div style="align-self: ${m.sender === 'user' ? 'flex-end' : 'flex-start'}; max-width: 85%;">
                            <div style="background:${m.sender === 'user' ? 'linear-gradient(135deg, #0ea5e9, #2563eb)' : '#334155'}; 
                                        color:#fff; padding:12px 18px; border-radius:${m.sender === 'user' ? '20px 20px 5px 20px' : '20px 20px 20px 5px'}; 
                                        font-size:16px; line-height:1.5; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
                                ${m.text}
                            </div>
                            <small style="font-size:12px; color:#475569; display:block; text-align:${m.sender === 'user' ? 'right' : 'left'}; margin-top:5px;">${m.time || ''}</small>
                        </div>
                    `).join('')}
                </div>
            </div>
...
<div class="gp-modal" id="gpDetailModal">
  ...
  <div class="gpd-mobile-view" style="display:none;">
    ...
    <div style="padding:0 14px;">
      ...
      <!-- ══ MOBILE FOOTER ══ -->
      <footer style="background:#111827;color:#d1d5db;padding:30px 20px 120px;border-top:1px solid #374151;font-family:'Hind Siliguri',sans-serif;margin-top:24px;">
        <!-- লোগো + স্লোগান -->
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
          ...
          <div>
            ...
            <p style="font-size:34px;margin:4px 0 0;color:#9ca3af;">আপনার বিশ্বস্ত অনলাইন শপ।</p>
          </div>
        </div>
        <!-- ২ কলাম: বাম = COMPANY, ডান = QUICK HELP + VERIFIED BY -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px;">
          <!-- বাম: COMPANY -->
          <div>
            <h3 style="color:#fff;font-size:39px;margin:0 0 12px;border-bottom:1px solid #374151;padding-bottom:8px;">COMPANY</h3>
            <ul style="list-style:none;padding:0;margin:0;font-size:34px;line-height:2.4;">
              <li><a href="javascript:void(0)" onclick="openAboutModal()" style="color:#9ca3af;text-decoration:none;">About Us</a></li>
              <li><a href="javascript:void(0)" onclick="openTermsModal()" style="color:#9ca3af;text-decoration:none;">Terms</a></li>
              <li><a href="javascript:void(0)" onclick="openPrivacyModal()" style="color:#9ca3af;text-decoration:none;">Privacy</a></li>
              <li><a href="javascript:void(0)" onclick="openReturnPolicyModal()" style="color:#9ca3af;text-decoration:none;">Return Policy</a></li>
              <li><a href="javascript:void(0)" onclick="openFaqModal()" style="color:#9ca3af;text-decoration:none;">FAQs</a></li>
            </ul>
          </div>
          <!-- ডান: QUICK HELP + VERIFIED BY -->
          <div>
            <h3 style="color:#fff;font-size:39px;margin:0 0 12px;border-bottom:1px solid #374151;padding-bottom:8px;">QUICK HELP</h3>
            <ul style="list-style:none;padding:0;margin:0 0 16px;font-size:34px;line-height:2.4;">
              <li><a href="javascript:void(0)" onclick="openCustomerCareModal()" style="color:#9ca3af;text-decoration:none;">গ্রাহক সেবা</a></li>
              <li><a href="javascript:void(0)" onclick="openFaqModal()" style="color:#9ca3af;text-decoration:none;">কিভাবে কিনবেন</a></li>
              <li><a href="javascript:void(0)" onclick="openReturnPolicyModal()" style="color:#9ca3af;text-decoration:none;">রিটার্ন ও রিফান্ড</a></li>
              <li><a href="tel:+8801707498418" style="color:#9ca3af;text-decoration:none;">📞 01707498418</a></li>
            </ul>
            <h3 style="color:#fff;font-size:39px;margin:0 0 10px;border-bottom:1px solid #374151;padding-bottom:8px;">VERIFIED BY</h3>
            <p style="font-size:34px;margin:0 0 4px;color:#9ca3af;">DBID: 437361334</p>
            <p style="font-size:34px;margin:0 0 12px;color:#9ca3af;">Reg: 304903094</p>
            <!-- পেমেন্ট মেথড -->
            <p style="font-size:34px;color:#6b7280;margin:0 0 8px;font-weight:700;">পেমেন্ট মেথড</p>
            <div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center;">
              <div style="background:#fff;border-radius:6px;padding:4px 6px;height:38px;display:flex;align-items:center;justify-content:center;"><img src="o1.jpg" alt="bKash" style="height:28px;width:auto;object-fit:contain;" onerror="this.parentElement.style.background='#E2136E';this.outerHTML='<span style=color:#fff;font-weight:900;font-size:26px;padding:0 6px>bKash</span>'"></div>
              <div style="background:#fff;border-radius:6px;padding:4px 6px;height:38px;display:flex;align-items:center;justify-content:center;"><img src="o2.png" alt="Nagad" style="height:28px;width:auto;object-fit:contain;" onerror="this.parentElement.style.background='#F15922';this.outerHTML='<span style=color:#fff;font-weight:900;font-size:26px;padding:0 6px>Nagad</span>'"></div>
              <div style="background:#fff;border-radius:6px;padding:4px 6px;height:38px;display:flex;align-items:center;justify-content:center;"><img src="o3.png" alt="Rocket" style="height:28px;width:auto;object-fit:contain;" onerror="this.parentElement.style.background='#8B008B';this.outerHTML='<span style=color:#fff;font-weight:900;font-size:26px;padding:0 6px>Rocket</span>'"></div>
              <div style="background:#fff;border-radius:6px;padding:4px 6px;height:38px;display:flex;align-items:center;justify-content:center;"><img src="o4.png" alt="Upay" style="height:28px;width:auto;object-fit:contain;" onerror="this.parentElement.style.background='#EE3424';this.outerHTML='<span style=color:#fff;font-weight:900;font-size:26px;padding:0 6px>Upay</span>'"></div>
              <div style="background:#fff;border-radius:6px;padding:4px 6px;height:38px;display:flex;align-items:center;justify-content:center;"><img src="o5.jpg" alt="DBBL" style="height:28px;width:auto;object-fit:contain;" onerror="this.parentElement.style.background='#006838';this.outerHTML='<span style=color:#fff;font-weight:900;font-size:26px;padding:0 6px>DBBL</span>'"></div>
              <div style="background:#fff;border-radius:6px;padding:4px 6px;height:38px;display:flex;align-items:center;justify-content:center;"><img src="o6.png" alt="City Bank" style="height:28px;width:auto;object-fit:contain;" onerror="this.parentElement.style.background='#003087';this.outerHTML='<span style=color:#fff;font-weight:900;font-size:26px;padding:0 6px>City</span>'"></div>
            </div>
          </div>
        </div>
        <!-- Copyright -->
        <div style="text-align:center;margin-top:20px;font-size:34px;color:#6b7280;border-top:1px solid #374151;padding-top:16px;">
          <p style="margin:0;">© 2026 <strong style="color:#d1d5db;">Digital Shop TM</strong>. সর্বস্বত্ব সংরক্ষিত।</p>
          <p style="margin:6px 0 0;">Developed by: <span style="color:#10b981;">Digital Shop TM Team</span></p>
        </div>
      </footer>
    </div>
...
function confirmFinalOrder(isCOD = false) {
   ...
   const newOrder = {
    ...
    setTimeout(() => {
        ...
        pop.style.cssText = 'position:fixed;inset:0;z-index:999999999;background:rgba(10,20,40,0.88);backdrop-filter:blur(12px);display:flex;align-items:center;justify-content:center;padding:' + f('16px','20px') + ';animation:osFadeIn 0.4s cubic-bezier(0.34,1.56,0.64,1);';
        ...
        pop.innerHTML = '<style>'
            ...
            + '@keyframes osPulse{0%,100%{box-shadow:0 0 0 0 rgba(34,197,94,0.4)}50%{box-shadow:0 0 0 22px rgba(34,197,94,0)}}'
            + '@keyframes osFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}'
            + '</style>'

            + '<div style="background:linear-gradient(160deg,#0f172a,#1a2744);border-radius:' + f('28px','32px') + ';padding:' + f('36px 28px','44px 30px') + ';max-width:' + f('360px','440px') + ';width:100%;text-align:center;border:1px solid rgba(255,255,255,0.1);box-shadow:0 32px 80px rgba(0,0,0,0.6),0 0 0 1px rgba(34,197,94,0.1);position:relative;overflow:hidden;">'

                // bg glow
                + '<div style="position:absolute;top:-80px;left:50%;transform:translateX(-50%);width:300px;height:300px;background:radial-gradient(circle,rgba(34,197,94,0.15),transparent 70%);border-radius:50%;pointer-events:none;"></div>'

                // check icon
                + '<div style="width:' + f('80px','110px') + ';height:' + f('80px','110px') + ';background:linear-gradient(135deg,#16a34a,#22c55e);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto ' + f('20px','26px') + ';animation:osPulse 2s infinite;">'
                +   '<svg width="' + f('40','56') + '" height="' + f('40','56') + '" viewBox="0 0 40 40" fill="none">'
                +     '<path d="M8 20 L16 28 L32 12" stroke="white" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="60" stroke-dashoffset="60" style="animation:osCheck 0.5s ease 0.3s forwards"/>'
                +   '</svg>'
                + '</div>'

                // badge
                + '<div style="background:rgba(34,197,94,0.12);border:1px solid rgba(34,197,94,0.3);display:inline-block;padding:' + f('4px 16px','8px 22px') + ';border-radius:20px;margin-bottom:' + f('14px','18px') + ';">'
                +   '<span style="color:#4ade80;font-size:' + f('12px','22px') + ';font-weight:700;letter-spacing:1px;">✅ অর্ডার সফল হয়েছে!</span>'
                + '</div>'

                + '<h2 style="color:#f8fafc;font-size:' + f('22px','36px') + ';font-weight:800;margin:0 0 8px 0;">ধন্যবাদ! 🎉</h2>'
                + '<p style="color:#94a3b8;font-size:' + f('13px','24px') + ';margin:0 0 ' + f('24px','30px') + ' 0;">আপনার অর্ডারটি সফলভাবে গ্রহণ করা হয়েছে।</p>'

                // order details box
                + '<div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:' + f('16px','20px') + ';padding:' + f('16px','22px') + ';margin-bottom:' + f('20px','26px') + ';text-align:left;">'
                +   '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;padding-bottom:12px;border-bottom:1px dashed rgba(255,255,255,0.08);">'
                +     '<span style="color:#64748b;font-size:' + f('12px','24px') + ';">অর্ডার আইডি</span>'
                +     '<span style="color:#60a5fa;font-size:' + f('13px','26px') + ';font-weight:700;">' + orderId + '</span>'
                +   '</div>'
                +   '<div style="display:flex;justify-content:space-between;align-items:center;' + (calculatedDiscount > 0 ? 'margin-bottom:12px;padding-bottom:12px;border-bottom:1px dashed rgba(255,255,255,0.08);' : '') + '">'
                +     '<span style="color:#64748b;font-size:' + f('12px','24px') + ';">পরিমাণ</span>'
                +     '<span style="color:#f1f5f9;font-size:' + f('13px','26px') + ';font-weight:600;">' + orderQty + ' পিস</span>'
                +   '</div>'
                + (calculatedDiscount > 0
                    ? '<div style="display:flex;justify-content:space-between;align-items:center;">'
                    +   '<span style="color:#64748b;font-size:' + f('12px','24px') + ';">ডিসকাউন্ট</span>'
                    +   '<span style="color:#4ade80;font-size:' + f('13px','26px') + ';font-weight:700;">-৳' + calculatedDiscount + '</span>'
                    + '</div>'
                    : '')
                + '</div>'

                // button
                + '<button onclick="document.getElementById(\'orderSuccessPop\').remove()" style="width:100%;background:linear-gradient(135deg,#16a34a,#22c55e);color:#fff;border:none;padding:' + f('14px','20px') + ';border-radius:' + f('14px','16px') + ';font-size:' + f('15px','28px') + ';font-weight:800;cursor:pointer;letter-spacing:0.5px;box-shadow:0 8px 20px rgba(34,197,94,0.3);">'
                +   'ঠিক আছে 👍'
                + '</button>'

                + '<p style="color:#475569;font-size:' + f('11px','22px') + ';margin:' + f('14px','18px') + ' 0 0 0;">My Account → Order Tracking থেকে ট্র্যাক করুন</p>'
    ...
    function getBoardsHtml() {
        ...
        return boards.map(b => `
            <div style="display:flex; align-items:center; gap:12px; background:#1f2937; border-radius:12px; padding:10px; margin-bottom:10px; border:1px solid #374151;">
                ...
                <button onclick="deleteNightBoard(${b.id})" style="background:#ef4444; color:#fff; border:none; width:32px; height:32px; border-radius:8px; cursor:pointer; flex-shrink:0;">
                    <i class="fa fa-trash"></i>
                </button>
            </div>`).join('');
    }
        ...
        <div style="background:#111827; color:#fff; width:100%; max-width:520px; border-radius:20px; border:1px solid #374151; box-shadow:0 25px 60px rgba(0,0,0,0.6); position:relative; overflow:hidden; max-height:90vh; display:flex; flex-direction:column;">
            ...
            <div style="padding:20px; overflow-y:auto; flex:1;">
                ...
                <div style="margin-bottom:18px;">
                    ...
                    <input type="url" id="nightLinkUrl" placeholder="https://..." style="width:100%; padding:12px; border-radius:10px; border:1px solid #374151; background:#1f2937; color:#fff; outline:none; box-sizing:border-box; font-size:13px;">
                </div>
                ...
                <div id="nightBoardList">${getBoardsHtml()}</div>
            </div>
        </div>
    </div>`;
...
function publishNightBoard() {
    ...
    if (listEl) {
        ...
        listEl.innerHTML = updated.length ? updated.map(b => `
            ...
            </div>`).join('') : '<p style="color:#6b7280; text-align:center; padding:20px;">কোনো নাইট বোর্ড নেই</p>';
    }
...
function publishSironam() {
    ...
    try {
        // _TM_FB_DBS থেকে fb7_ads সরাসরি নাও — fallback/ready-check এড়াতে
        ...
        if (_sDB) {
            ...
            console.warn('[FB] ❌ fb7_ads DB পাওয়া যায়নি — sironam save হয়নি!');
        }
    } catch(e) {
...
function displaySironamOnPortal() {
    ...
    container.innerHTML = sironamData.map(item => {
        // এই শিরোনামে কোনো পণ্য আছে কিনা চেক
        ...
        if (hasProducts) {
            ...
            // পণ্য নেই → lock করা কার্ড
            return `
            <div class="sironam-card" style="cursor:not-allowed; opacity:0.55; filter:grayscale(40%); pointer-events:none; position:relative;">
                <div style="position:absolute; top:6px; right:6px; background:#ef4444; color:#fff; font-size:10px; font-weight:700; padding:2px 7px; border-radius:20px; font-family:'Hind Siliguri',sans-serif; z-index:2;">শীঘ্রই</div>
                <img src="${item.img}" alt="${item.name}">
                <div class="sironam-overlay">
                    <span>${item.name}</span>
                </div>
...
function openSironamShop(id, title) {
    ...
    <div id="sironamFullShop" style="position:fixed; top:0; left:0; width:100%; height:100%; background:${_shopBg}; z-index:999999999; overflow-y:auto; font-family: 'Hind Siliguri', sans-serif;">
        ...
        <div id="shopAllProductsWrapper" style="background:${_isDark ? '#dcfce7' : '#fef9c3'}; margin:0; padding:0;">
            ...
            <div id="shopProductGrid" style="display:grid; grid-template-columns:${window.screen.width < 800 ? 'repeat(2, 1fr)' : 'repeat(auto-fill, minmax(220px, 1fr))'}; gap:${window.screen.width < 800 ? '10px' : '20px'}; padding:${window.screen.width < 800 ? '12px' : '24px 30px 30px'};">
    ...
    return filtered.map(p => {
        ...
        <div class="shop-product-item" data-name="${p.title || p.name}" data-tags="${Array.isArray(p.tags)?p.tags.join(','):(p.tags || p.sironamTag || '')}"
            ...
            <div style="width:100%;aspect-ratio:1/1;overflow:hidden;border-radius:10px;background:${_imgBg};">
                <img src="${displayImg}" style="width:100%;height:100%;object-fit:fill;display:block;">
            </div>
...
function pmxOpenHeaderShop(headerId, headerName, headerImg) {
    ...
    const titleClr   = isDark ? '#a78bfa' : '#4f46e5';
    const srchBtnBg  = isDark ? '#1e293b' : '#ffffff';
    const srchBtnBdr = isDark ? '#475569' : '#d1d5db';
    const srchBtnClr = isDark ? '#ffffff' : '#1a1a1a';
    const srchExpBg  = isDark ? '#1e293b' : '#f8fafc';
    const srchExpBdr = isDark ? '#334155' : '#e2e8f0';
    const srchBoxBg  = isDark ? '#0f172a' : '#f1f5f9';
    const srchInpClr = isDark ? '#e2e8f0' : '#1e293b';
    const srchIconClr= isDark ? '#94a3b8' : '#64748b';
    const cardInfoBg = isDark ? '#1a3a2a' : '#c7d9f0';
    const nameFontClr= isDark ? '#e2e8f0' : '#1e293b';
    const priceFontClr= isDark ? '#10b981' : '#0369a1';
    ...
    <div id="pmxHeaderShopModal" style="position:fixed;inset:0;background:${modalBg};z-index:999999999;overflow-y:auto;font-family:'Hind Siliguri',sans-serif;">
        ...
        <!-- হোল্ডার বোর্ড -->
        <div id="pmxHolderBoard" style="width:100%;max-width:1520px;height:350px;margin:20px auto;background:#0a0a0a;border-radius:0;overflow:hidden;border:1px solid #334151;position:relative;transition:all 0.3s ease;">
            ${holderSlides}
            ${holders.length > 1 ? `
        ...
        <!-- সকল পণ্য wrapper — হালকা bg, এখান থেকে footer পর্যন্ত -->
        <div style="background:${cardInfoBg};">
            <!-- সকল পণ্য হেডিং -->
            <div style="padding:10px 20px 0 20px;">
                <span style="font-family:'Georgia',serif;font-size:clamp(14px,2.5vw,22px);font-weight:900;letter-spacing:0.06em;text-transform:uppercase;color:${isDark?'#14532d':'#1e3a5f'};display:inline-block;border-bottom:3px solid ${isDark?'#86efac':'#60a5fa'};background:${isDark?'#dcfce7':'#dbeafe'};padding:4px 14px;border-radius:6px;">
                    সকল পণ্য
                </span>
            </div>
            <!-- সরু কালো বার -->
            ...
            <!-- Footer -->
            <footer style="background:#111827;color:#d1d5db;padding:50px 20px 20px;border-top:1px solid #374151;font-family:sans-serif;margin-top:10px;">
                <div style="max-width:1200px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:40px;">
                    <div>
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:15px;">
                            <img src="ko.jpeg" alt="Logo" style="width:48px;height:48px;border-radius:10px;object-fit:fill;border:2px solid #10b981;" onerror="this.style.display='none'">
...
function pmxToggleShopSearch() {
    ...
    if (!isOpen) {
        ...
        // সার্চ বার বন্ধ — ইনপুট খালি করে সব রিসেট
        const inp = document.getElementById('pmxShopSearchInput');
        if (inp) inp.value = '';
...
function pmxFilterShopProducts() {
    ...
    // হোল্ডার বোর্ড hide/show
    const holderBoard = document.getElementById('pmxHolderBoard');
    if (holderBoard) {
        if (term !== '') {
            holderBoard.style.display = 'none';
        } else {
            holderBoard.style.display = 'block';
        }
    }
...
// ══════════════════════════════════════════════════════════════════
/* ═══════════════════════════════════════════════════════════════════
   PMX PREMIUM PRODUCT DETAILS — Full-Screen v3.0
   PC: ছবি বামে, তথ্য ডানে | Mobile: ছবি উপরে, তথ্য নিচে
   নাইট মোড সাপোর্ট | জেনারেটিভ সেল কাউন্টার (১০০০–২০,০০,০০,০০০)
   পুরনো buy modal এর সব ফাংশন অক্ষুণ্ণ
   ═══════════════════════════════════════════════════════════════════ */
function pmxOpenBuyModal(productId) {
    ...
    const priceC   = isDark ? '#34d399' : '#059669';
    const inputBg  = isDark ? '#0f172a' : '#f1f5f9';
    const inputClr = isDark ? '#e2e8f0' : '#1e293b';
    const inputBdr = isDark ? '#334155' : '#cbd5e1';
    const sectionBg= isDark ? '#1a2744' : '#f0f9ff';
    const sectionBdr= isDark ? '#2d4a6a' : '#bfdbfe';
    const labelC   = isDark ? '#7dd3fc' : '#1d4ed8';
    const accentGold= '#f5c518';

    // ── জেনারেটিভ সেল কাউন্টার ──
    // প্রতিটি পণ্যের জন্য unique seed → সংখ্যা বাড়বে, max হলে reset
    const _pid = String(p.id || p.name || '');
    const _seed = _pid.split('').reduce((a,c,i)=>a+c.charCodeAt(0)*(i+3),0);
    const _storageKey = 'pmx_salecnt_' + _pid;
    const _min = 1000;
    const _max = 200000000; // ২০ কোটি
    let _curSale = parseInt(localStorage.getItem(_storageKey) || '0');
    if (!_curSale || _curSale < _min) {
        // প্রথমবার: seed থেকে _min-_max এর মধ্যে random শুরু
        _curSale = _min + (_seed % (_max - _min));
    }
    // প্রতি ওপেনে random ৩–৪৭ যোগ হয়
    const _increment = 3 + (_seed % 45);
    _curSale += _increment;
    if (_curSale >= _max) _curSale = _min + (_seed % 50000); // reset
    localStorage.setItem(_storageKey, String(_curSale));
    const _saleDisplay = _curSale.toLocaleString('bn-BD');

    // ── পুরনো price/discount লজিক ──
    const _dpSeed2 = _pid.split('').reduce((acc,c,i)=>acc+c.charCodeAt(0)*(i+7),0);
    let dispOldPrice = parseFloat(p.origPrice || p.oldPrice || 0);
    if (!dispOldPrice || dispOldPrice <= parseFloat(p.price)) {
        const _opSeed = _dpSeed2 % 1000;
        const multiplier = 1.12 + (_opSeed / 1000) * 0.22;
        dispOldPrice = Math.ceil(parseFloat(p.price) * multiplier / 10) * 10;
    }
    const discPct = dispOldPrice > parseFloat(p.price)
    ...
    const spRevCnt = 50 + (spHash2 % 950);
    const fullStars2 = Math.floor(spRating);
    const halfStar2 = (spRating - fullStars2) >= 0.5;
    let starsHtml2 = '';
    for(let i=0;i<fullStars2;i++) starsHtml2+='<span style="color:#f59e0b;">★</span>';
    if(halfStar2) starsHtml2+='<span style="color:#f59e0b;font-size:0.9em;">½</span>';
    for(let i=fullStars2+(halfStar2?1:0);i<5;i++) starsHtml2+='<span style="color:#d1d5db;">★</span>';

    // ── PC লেআউট ──
    const pcLayout = `
    <div style="display:flex;height:100%;overflow:hidden;">
      <!-- বাম: ছবি -->
      <div style="width:48%;flex-shrink:0;background:${isDark?'#111827':'#f9fafb'};display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px 30px;border-right:1px solid ${borderC};">
        <div style="width:100%;max-width:440px;border-radius:16px;overflow:hidden;border:2px solid ${borderC};box-shadow:0 8px 32px rgba(0,0,0,0.18);background:${cardBg};aspect-ratio:1/1;display:flex;align-items:center;justify-content:center;">
          <img src="${p.img||''}" id="pmxDetImg" style="width:100%;height:100%;object-fit:contain;display:block;" onerror="this.src='ko.jpeg'">
        </div>
        <!-- সেল কাউন্টার ব্যাজ -->
        <div style="margin-top:20px;background:linear-gradient(135deg,${isDark?'#1e3a2a':'#ecfdf5'},${isDark?'#0d2a1a':'#d1fae5'});border:1.5px solid ${isDark?'#34d399':'#10b981'};border-radius:16px;padding:14px 24px;text-align:center;width:100%;max-width:440px;box-sizing:border-box;">
          <div style="color:${mutedC};font-size:12px;font-weight:600;margin-bottom:4px;letter-spacing:0.5px;">🔥 সর্বমোট অর্ডার করেছেন</div>
          <div id="pmxSaleCntPC" style="color:${priceC};font-size:28px;font-weight:900;font-family:'Hind Siliguri',sans-serif;letter-spacing:1px;">${_saleDisplay} জন</div>
          <div style="color:${mutedC};font-size:11px;margin-top:3px;">এই পণ্য সংখ্যা বাড়ছে ↑</div>
        </div>
      </div>
      <!-- ডান: তথ্য -->
      <div style="flex:1;overflow-y:auto;padding:0;background:${pageBg};">
        <!-- sticky top bar -->
        <div style="position:sticky;top:0;background:${headerBg};backdrop-filter:blur(12px);border-bottom:1px solid ${borderC};padding:14px 28px;display:flex;align-items:center;justify-content:space-between;z-index:10;">
          <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div style="padding:28px 32px 40px;">
          <!-- নাম -->
          <h1 style="color:${titleC};font-size:22px;font-weight:800;margin:0 0 12px;line-height:1.5;font-family:'Hind Siliguri',sans-serif;">${p.name||''}</h1>
          <!-- স্টার -->
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:16px;">
            <div style="font-size:18px;">${starsHtml2}</div>
            <span style="color:${accentGold};font-size:14px;font-weight:700;">${spRating.toFixed(1)}</span>
            <span style="color:${mutedC};font-size:13px;">(${spRevCnt} রিভিউ)</span>
          </div>
          <!-- দাম -->
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;flex-wrap:wrap;">
            <span style="color:${priceC};font-size:36px;font-weight:900;font-family:'Hind Siliguri',sans-serif;">৳${p.price}</span>
            ${dispOldPrice?`<span style="color:${mutedC};font-size:18px;text-decoration:line-through;">৳${dispOldPrice}</span>`:''}
            ${discPct?`<span style="background:#ef4444;color:#fff;font-size:13px;font-weight:800;padding:4px 12px;border-radius:6px;">${discPct}% ছাড়</span>`:''}
          </div>
          <!-- সেল কাউন্টার মিনি -->
          <div style="display:inline-flex;align-items:center;gap:8px;background:${isDark?'rgba(52,211,153,0.1)':'rgba(16,185,129,0.07)'};border:1px solid ${isDark?'#065f46':'#6ee7b7'};border-radius:10px;padding:8px 16px;margin-bottom:20px;">
            <span style="font-size:18px;">🛒</span>
            <span style="color:${priceC};font-weight:800;font-size:15px;" id="pmxSaleCntMiniPC">${_saleDisplay}</span>
            <span style="color:${mutedC};font-size:12px;">জন অর্ডার করেছেন</span>
          </div>
          <!-- বিস্তারিত -->
          ${p.desc?`
          <div style="background:${isDark?'#1e293b':'#f8fafc'};border:1px solid ${borderC};border-radius:12px;padding:16px;margin-bottom:20px;">
            <div style="color:${isDark?'#7dd3fc':'#1d4ed8'};font-size:13px;font-weight:700;margin-bottom:8px;">📋 বিস্তারিত বিবরণ</div>
            <div style="color:${isDark?'#cbd5e1':'#475569'};font-size:14px;line-height:1.8;">${p.desc}</div>
          </div>`:''}
          <!-- অর্ডার ফর্ম -->
          <div style="background:${sectionBg};border:1.5px solid ${sectionBdr};border-radius:16px;padding:20px;">
            ...
            <div style="display:flex;gap:10px;margin-bottom:12px;">
              <label onclick="pmxSelectPayment('bkash')" id="pmxPayBkash" style="flex:1;display:flex;align-items:center;gap:8px;padding:12px;background:${inputBg};border:2px solid #e91e8c;border-radius:10px;cursor:pointer;">
                ...
                <span style="color:${inputClr};font-size:14px;font-weight:700;">💳 বিকাশ</span>
              </label>
              <label onclick="pmxSelectPayment('nagad')" id="pmxPayNagad" style="flex:1;display:flex;align-items:center;gap:8px;padding:12px;background:${inputBg};border:2px solid ${inputBdr};border-radius:10px;cursor:pointer;">
                ...
                <span style="color:${inputClr};font-size:14px;font-weight:700;">💳 নগদ</span>
              </label>
            </div>
    ...
    <div style="width:100%;background:${isDark?'#111827':'#f9fafb'};aspect-ratio:1/1;display:flex;align-items:center;justify-content:center;max-height:75vw;overflow:hidden;border-bottom:1px solid ${borderC};">
      <img src="${p.img||''}" style="width:100%;height:100%;object-fit:contain;display:block;" onerror="this.src='ko.jpeg'">
    </div>
    <!-- তথ্য নিচে -->
    ...
    window._pmxSaleCntTimer = setInterval(() => {
        ...
        if (!modal) { clearInterval(window._pmxSaleCntTimer); return; }
        let cur = parseInt(localStorage.getItem(_storageKey) || String(_curSale));
        cur += 1 + Math.floor(Math.random() * 3);
        if (cur >= _max) cur = _min + (_seed % 50000);
        localStorage.setItem(_storageKey, String(cur));
        const fmt = cur.toLocaleString('bn-BD') + ' জন';
        const e1 = document.getElementById('pmxSaleCntPC');
        const e2 = document.getElementById('pmxSaleCntMiniPC');
        if (e1) { e1.textContent = fmt; e1.style.transform='scale(1.1)'; setTimeout(()=>{if(e1)e1.style.transform='';},300); }
        if (e2) { e2.textContent = cur.toLocaleString('bn-BD'); }
    }, 5000);
}
...
function pmxPlaceOrder(productId) {
    ...
    if (window._pmxSaleCntTimer) clearInterval(window._pmxSaleCntTimer);
